...
public class AccountingDaoImpl implements AccountingDao {
    ...
    public List<Accounting> getAccountingList() {
        List<Accounting> accountingList= new LinkedList<>();
        try (Statement statement = connection.createStatement();
             ResultSet resultSet = statement.executeQuery(
                     "SELECT " +
                             " payment_number," +
                             ...
                             " WHERE recipient_id = contractors.id;")) {
            while (resultSet.next()) {
                    Accounting row = new Accounting();
                    row.setPaymentNumber(resultSet.getInt("payment_number"));
                    row.setPaymentDate(resultSet.getDate("payment_date"));
                    row.setEntityName(resultSet.getString("entity_name"));
    ...
    public List<Accounting> getAccountingList(PrintWriter printWriter) {
        ...
        try (Statement statement = connection.createStatement();
            ...
            printWriter.println("Database request error. Please contact your support service");
        }
        return accountingList;
    }
